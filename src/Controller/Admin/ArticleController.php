...
class ArticleController extends EntityController
{
    ...
    ): Response
    {
        ...
        $form = $this->createForm(ArticleType::class, $entity, ['validation_groups' => ['persist']]);
    ...
    ): Response
    {
        ...
        $form = $this->createForm(ArticleType::class, $entity, ['validation_groups' => ['update']]);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            $this->entityManager->flush();
            return $this->redirectToRoute('admin_'.$this->getEntityShortname(true).'_index', [], Response::HTTP_SEE_OTHER);
