...
class RelationMapper implements RelationMapperInterface
{
    ...
    public readonly EntitiesDescriptorInterface $eDescriptor;
    ...
    public function __construct(
        ...
        $this->eDescriptor = $this->wireEm->getEntitiesDescriptor();
    ...
    private function computeRelatedDependencies(): void
    {
        ...
        foreach ($relative_mappings as $field => $mapping) {
            ...
            $this->relations[$field]['require_all'] = $this->eDescriptor->findAll($relative_mappings[$field]['require'], andOperator: false);
            $this->relations[$field]['require_instantiable'] = $this->eDescriptor->enableFilterFinal(true)->findAll($relative_mappings[$field]['require'], andOperator: false);
            $this->relations[$field]['require_metadata'] = $cmd_mappings[$mapping['field']]->targetEntity;
            $this->relations[$field]['has_between'] = $this->eDescriptor->isBetweenEntity($cmd_mappings[$mapping['field']]->targetEntity);
            ...
            $this->relations[$field]['isToOne'] = $this->classMetadata->getAssociationMapping($mapping['field'])->isToOne();
            $this->relations[$field]['isToMany'] = $this->classMetadata->getAssociationMapping($mapping['field'])->isToMany();
        ...
        foreach ($cmd_mappings as $field => $mapping) {
            $this->relations[$field] = array_merge([
                ...
                'mapping' => $mapping,
                'require_all' => $this->eDescriptor->findAll([$mapping->targetEntity]),
                'require_instantiable' => $this->eDescriptor->enableFilterFinal(true)->findAll([$mapping->targetEntity]),
                'require_metadata' => $mapping->targetEntity,
                'has_between' => $this->eDescriptor->isBetweenEntity($mapping->targetEntity),
                'described_by' => 'metadata',
                'classname' => $this->classMetadata->name,
                'shortname' => $this->classMetadata->getReflectionClass()->getShortname(),
                'isToOne' => $mapping->isToOne(),
                'isToMany' => $mapping->isToMany(),
        ...
        //     // Is a between entity
        //     if($this->eDescriptor->isBetweenEntity($this->relations[$field]['require_metadata'])) {
        ...
        //         $require_all_instantiables = $this->eDescriptor->findAll($mapping['require_metadata'], andOperator: false);
        ...
        //         if(!$this->eDescriptor->isBetweenEntity($between->name)) {
        ...
        //                     $this->relations[$field]['require_all'] = $this->eDescriptor->findAll([$mapp->targetEntity], andOperator: false);
        //                 }
        ...
        foreach ($this->relations as $field => $mapping) {
            if(empty($mapping['require_all'])) {
                // No require classes for this relation
                $this->addError(vsprintf('Error %s line %d: relation property "%s" of %s has no require classes!%s- Please define at least one class!', [__METHOD__, __LINE__, $field, $this->classname, PHP_EOL]));
            }
            if(empty($mapping['require_instantiable'])) {
                // No instantiable classes for this relation
                $this->addError(vsprintf('Error %s line %d: relation property "%s" of %s has no instantiable classes!%s- Please define at least one class!', [__METHOD__, __LINE__, $field, $this->classname, PHP_EOL]));
            }
        }
    }
    ...
    private function getRelativeAssociationMappings(): array
    {
        if(!static::SERIALIZATION_MAPPINGS_BY_ATTRIBUTE) {
            ...
            $mappings = Objects::getClassAttributes($this->classname, WireRelationMapping::class);
            // Get first mapping
            /** @var WireRelationMapping|false $mapping */
            $mappings = reset($mappings);
        }
        return $mappings instanceof WireRelationMapping ? $mappings->getMapping() : [];
    }
